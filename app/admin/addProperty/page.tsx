...
/* ------------------ Types that don't need bedroom/bathroom/furnishing ------------------ */
const PROPERTY_TYPES_NO_ROOMS = ["Plot", "Commercial"];
...
export default function AddPropertyPage() {
  ...
  /* Helper: does current propertyType need rooms? */
  const showRoomFields = !PROPERTY_TYPES_NO_ROOMS.includes(formData.propertyType);
  ...
  const handleSelect = (key, value) => {
    ...
    // Clear room-related fields when switching to a no-room type
    if (key === "propertyType" && PROPERTY_TYPES_NO_ROOMS.includes(value)) {
      updated.bedrooms = "";
      updated.bathrooms = "";
      updated.furnishing = "";
    }
        ...
        new Blob(
          [
            JSON.stringify({
              ...
              price: Number(formData.price),
              area: Number(formData.area),
              // Only send numeric room fields when applicable
              ...(showRoomFields && {
                bedrooms: Number(formData.bedrooms),
                bathrooms: Number(formData.bathrooms),
              }),
          ...
          <form onSubmit={handleSubmit}>
            ...
            {activeTab === "basic" && (
              <SectionCard title="Basic Information" icon={Home} delay={0.1}>
                ...
                {/* Furnishing: hidden for Plot / Commercial */}
                {showRoomFields && (
                  <SelectBlock
                    label="Furnishing Status"
                    options={["Fully Furnished", "Semi-Furnished", "Unfurnished"]}
            ...
            {activeTab === "details" && (
              <SectionCard title="Property Details" icon={Building} delay={0.2}>
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  {/* Bedrooms & Bathrooms: hidden for Plot / Commercial */}
                  {showRoomFields && (
                    <>
                      <InputBlock
                        label="Bedrooms"
