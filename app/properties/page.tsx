...
/* Types + normalization                                                      */
/* -------------------------------------------------------------------------- */
/* Your API returns raw fields (propertyFor, propertyType, ownerName, ...).   */
/* The old code read `p.type` for status, which doesn't exist on the raw     */
/* payload — the status filter and the "For Rent/For Sell" badge were        */
/* silently broken. normalizeProperty() maps once, here, so every filter/    */
/* sort/render downstream works off a consistent shape.                      */
interface NormalizedProperty {
  ...
  image: string;
  type: "rent" | "sale";
  category: string; // Apartment / Villa / Plot / Commercial ...
  price: number;
  bedrooms: number;
  bathrooms: number;
  area: number;
...
const CACHE_KEY = "properties_cache_v2";
const PAGE_SIZE = 12;
...
/* Per-card image with its own skeleton so slow images never block the grid  */
/* from painting — text, price, and layout show immediately; each photo     */
/* fades in independently as it finishes downloading.                       */
function CardImage({
  src,
  alt,
  priority,
...
/* Two-thumb range control: drag the slider OR type exact numbers in the      */
/* min/max boxes. Typed values are clamped so min never exceeds max and      */
/* both stay within [min, max] bounds — the old version only showed the      */
/* numbers as static text with no way to enter a precise value.              */
function RangeControl({
  ...
  // Local text state so the input isn't fighting the user mid-keystroke
  // (e.g. clearing the field to type a new number) before it's committed.
...
function PropertiesDashboard() {
  ...
  // Fetch properties — stale-while-revalidate: paint instantly from cache,
  ...
  useEffect(() => {
    ...
    const fetchProperties = async () => {
      ...
      try {
        ...
        // Corrupted cache — ignore and fall through to network fetch.
      }

      try {
        const res = await fetch(`${BASE_URL}/properties/accepted`, { signal: controller.signal });
        if (!res.ok) throw new Error("Failed to fetch properties");
        const data = await res.json();
  ...
  // Reset pagination whenever the active filter/sort set changes, so you
  // don't end up on "page 3" of a now much-shorter filtered list.
  useEffect(() => {
    setVisibleCount(PAGE_SIZE);
  }, [search, selectedTypes, selectedStatuses, bedrooms, priceRange, areaRange, sortBy]);
  ...
  // Loading Skeleton — only for the true first-load case (no cache available)
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col bg-gray-50">
        <Navbar />
      ...
      <main className="flex-1">
        ...
        <section className="relative bg-gradient-to-r from-blue-600 to-blue-800 text-white py-12 overflow-hidden">
          ...
          <div className="container px-4 md:px-6 relative z-10">
            ...
            >
              ...
              <p className="text-base text-blue-100">
                {t.Browseourlistingstofindyourperfectproperty ?? "Browse our extensive collection of premium properties"}
              </p>
            </motion.div>
          </div>
        </section>
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                  ...
                  <div>
                    <h3 className="font-semibold mb-3 text-sm">{t.QuickFilters ?? "Quick Filters"}</h3>
                    <div className="flex flex-wrap gap-2">
                      <Badge variant="outline" className="cursor-pointer hover:bg-primary hover:text-white transition-colors" onClick={() => setPriceRange([0, 5000000])}>
                        Under ₹50L
                      </Badge>
                      <Badge variant="outline" className="cursor-pointer hover:bg-primary hover:text-white transition-colors" onClick={() => setPriceRange([5000000, 10000000])}>
                        ₹50L - ₹1Cr
                      </Badge>
                      <Badge variant="outline" className="cursor-pointer hover:bg-primary hover:text-white transition-colors" onClick={() => setPriceRange([10000000, 20000000])}>
                        ₹1Cr - ₹2Cr
                      </Badge>
                      <Badge variant="outline" className="cursor-pointer hover:bg-primary hover:text-white transition-colors" onClick={() => setBedrooms("2")}>
                        2 BHK
                      </Badge>
                      <Badge variant="outline" className="cursor-pointer hover:bg-primary hover:text-white transition-colors" onClick={() => setBedrooms("3")}>
                        3 BHK
                      </Badge>
                    </div>
                  </div>
        ...
        <section className="py-8">
          <div className="container px-4 md:px-6">
                        ...
                        <CardImage src={property.image} alt={property.title} priority={index < 3}>
                          <div className="absolute top-3 left-3 z-10">
                            <Badge
                              className={cn(
                                "px-2 py-1 text-xs font-semibold",
                                property.type === "rent" ? "bg-blue-500 hover:bg-blue-600" : "bg-green-500 hover:bg-green-600"
                              )}
                            >
            ...
            {!error && filteredProperties.length > visibleCount && (
              <div className="text-center mt-12">
                <Button variant="outline" size="lg" className="px-8" onClick={() => setVisibleCount((c) => c + PAGE_SIZE)}>
                  {t.LoadMoreProperties ?? "Load More Properties"} ({filteredProperties.length - visibleCount} {t.remaining ?? "more"})
                </Button>
              </div>
            )}
          </div>
